...
static double idp_computesol (unsigned long int, int *, unsigned long int *,
			      unsigned long int *, double *, int);
static int idp_next_coalition (int *, int);
//static int idp_next_split_structure (int *, int);
static int idp_next_split_structure1 (int *, int, int);
...
int
idp_next_split_structure1 (int *input, int n, int k)
{
  int i = n - 1;
  int ones = 0;
  int c;
  ...
  */
  ...
  /*   printf("\n ;");
     ...
     printf("%d ",input[k1]); 
  */
	if (input[0] == 0 && (double) n/2 == (double) k)
		return(0);
...
/*
static int
idp_next_split_structure (int *input, int n)
{
  int i = 0;
...
int
idp_next_coalition (int *input, int n)
{
  int i = n - 1, k;
...
double
idp_computesol (unsigned long int coal, int *splitted, unsigned long int *f11,
		unsigned long int *f12, double *f2, int nAgents)
{
  ...
  if (splitted[coal])
    {
      val = idp_computesol (f11[coal], splitted, f11, f12, f2, nAgents) +
	idp_computesol (f12[coal], splitted, f11, f12, f2, nAgents);
    }
  else
    {
      printf ("{");
...
idp (const int nAgents)
{
  ...
  int *splitted;
  if ((splitted =  (int *) calloc ((pow (2, nAgents) - 1), sizeof (int))) == NULL)    {
		printf ("\nImpossible to allocate %f sizeof (int)\n",pow (2, nAgents) - 1);
		exit (1);
	}
  unsigned long int *f11;
  if ((f11 = (unsigned long int *) calloc ((pow (2, nAgents) - 1), sizeof (unsigned long int))) == NULL)    {
  ...
  for (cl = 2; cl <= nAgents; cl++)    {
      ...
      /* imposto la coalizione iniziale */
		...
		do{
			...
			while (s1 <= s2)	{
				//  printf("\ncl %d, s1 %d, s2 %d ",cl,s1,s2);
							
				if (s2 <= (nAgents - cl) || cl == nAgents)	{
					for (k = 0; k < s1; k++)
						coalSplit[k] = 1;
					for (k = s1; k < nAgents; k++)
						coalSplit[k] = 0;
		       ...
		       */
						...
						if (__verbose)
							printf (" %lld:%f %lld:%f %f", index1, f2[index1],index2, f2[index2], f2[index1] + f2[index2]);

						double sum = f2[index1] + f2[index2];
						if (sum > maxVal)	{
							maxVal = sum;
							maxSplit = 1;
							maxf11 = index1;
					...
					}	while (idp_next_split_structure1 (coalSplit, cl, s1));
				}
	      s1++;
	      s2--;
	    }
	      ...
	      if (maxSplit)
			...
			f12[index] = maxf12;
			f2[index] = maxVal;
		} while (idp_next_coalition (coal, nAgents));
  ...
  printf ("\n>Solution: ");
  best = idp_computesol (start - 1, splitted, f11, f12, f2, nAgents);
